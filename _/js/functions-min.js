...
//Scroll to button
s("body").on(e,"button#jump-2-form",function(){return s("html, body").animate({scrollTop:s("section#form-section").offset().top},500),!1}),
/* 	MAIN TOP NAVIGATION FUNCTIONS 
			Button functions to open top navigation and 
			control the internal menu functions as well
			as close the menu.
		*/
s("body").on(e,"button#nav-btn",function(){var t=s("#top-nav").find(".nav-wrapper").outerHeight();return 1===s("#recent-blog-sb").length&&s("#recent-blog-sb").removeClass("sb-closed sb-open").addClass("sb-hidden"),1===s("#xmas-popup-btn-wrap").length&&s("#xmas-popup-btn-wrap").removeClass("pop-up-inactive").addClass("pop-up-active"),s("#top-nav").animate({height:t+"px"},300,function(){s("body").toggleClass("nav-closed nav-open"),s(this).toggleClass("nav-closed nav-open").removeAttr("style")}),!1}),s("#top-nav").on(e,"button#close-nav",function(){return s("#top-nav").animate({height:"0px"},300,function(){s("body").toggleClass("nav-open nav-closed"),s(this).toggleClass("nav-open nav-closed").removeAttr("style"),s("li.with-sub-nav").removeClass("sub-open").addClass("sub-closed"),1===s("#recent-blog-sb").length&&s("#recent-blog-sb").removeClass("sb-hidden").addClass("sb-closed"),1===s("#xmas-popup-btn-wrap").length&&s("#xmas-popup-btn-wrap").removeClass("pop-up-active").addClass("pop-up-inactive")}),!1}),s("#top-nav").on("click","li.with-sub-nav > a",function(){var t=s(this).parent(),n=s(t).siblings();return s(t).siblings().removeClass("sub-open").addClass("sub-closed"),0<s(n).find(".sub-open").length&&s(n).find(".sub-open").removeClass("sub-open").addClass("sub-closed"),
...
/* 	HEADER SEARCH BUTTON 
		...
		*/
s("body").on(e,"button#search-btn",function(){return s("#search-pop-up").toggleClass("off on"),s("body").addClass("search-open"),!1}),s("body").on(e,"button#close-search",function(){return s("#search-pop-up").hasClass("on")&&(s("#search-pop-up").removeClass("on").addClass("turn-off"),s(".turn-off").one("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend",function(){s(this).removeClass("turn-off").addClass("off"),s("body").removeClass("search-open")})),!1}),
/* END OF HEADER SEARCH FUNCTIONS */
/* 	BLOG SLIDE IN SIDEBAR BUTTON
			Button functions for blog slide in section
		*/
s("body").on(e,"button#sb-actions-btn",function(){return s(".slide-in-right").toggleClass("sb-closed sb-open"),!1}),
/* END OF BLOG SLIDE IN SIDEBAR BUTTON FUNCTIONS */
/* 	ROUTE FINDER BUTTON 
			Button functions for route finder pop up
		*/
s("body").on(e,"button#close-route-finder",function(){return s("#route-finder-pop-up").hasClass("on")&&(s("#route-finder-pop-up").removeClass("on").addClass("turn-off"),s(".turn-off").one("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend",function(){s(this).removeClass("turn-off").addClass("off"),s("body").removeClass("route-finder-open")})),!1}),
...
/* ACCESSABILITY FUNCTIONS 
	    ...
	    */
s("body").on(e,"#txt-only-content button.access-btn",function(){if(s(this).data().size){var t=s(this).attr("data-size");s(this).siblings().not("#access-btn-brightness").removeClass("active"),s(this).addClass("active"),s(".main-txt").removeClass("txt-md txt-lg txt-sm").addClass(t)}if(s(this).data("status")){s(this).toggleClass("off on");var n=s(this).data().status,e;console.log(n),"bg-white"==n&&(e="bg-black",s("#txt-only-content").removeClass("bg-white").addClass(e),s(this).attr("data-status",e).data("status",e)),"bg-black"==n&&(e="bg-white",s("#txt-only-content").removeClass("bg-black").addClass("bg-white"),s(this).attr("data-status",e).data("status",e)),console.log(e)}return!1}),
/* PAGE TOOLS BTNS */
s("body").on(e,"button#print-pg-btn",function(){n()}),
/* TEXT ONLY FUNCTION */
s("body").on(e,"button#txt-only-btn",function(){var t=s(".content-section").find(".main-txt"),n=s(".banner-title .container").text(),e=s(t).clone(),o=s("#txt-only-content").innerHeight()-170,a;n&&s('<div class="title-header">'+n+"</div>").appendTo("#txt-only-wrapper");return s(e).appendTo("#txt-only-wrapper").slimScroll({height:o+"px"}),s("#txt-only-wrapper").fadeIn("fast"),s("body").addClass("txt-only-open"),s("#txt-only-content").removeClass("closed").addClass("open"),!1}),s("body").on(e,"button#close-txt-only-btn",function(){return s("#txt-only-wrapper").fadeOut("fast").empty(),s("#txt-only-content").removeClass("open").addClass("closed"),s("body").removeClass("txt-only-open"),s("#txt-only-wrapper").hasClass("bg-black")&&s("#txt-only-wrapper").removeClass("bg-black").addClass("bg-white"),!1});
/* POP UP FUNCTIONS */
/* XMAS Pop up Function
		This function controls the Xmas pop up box
	    */
var t=function(){1===s("#xmas-popup-wrap").length&&s("#xmas-popup-wrap").hasClass("pop-up-inactive")&&(s("#xmas-popup-btn-wrap").removeClass("pop-up-inactive").addClass("pop-up-active"),s("#xmas-popup-wrap").removeClass("pop-up-inactive").addClass("pop-up-active"),s("#xmas-popup-wrap").fadeIn("slow",function(){s(".xmas-popup-inner").removeClass("hidden").addClass("animated slideInUp")}))};
...
//Button actions
s("body").on(e,"button#xmas-popup-btn-open",function(){return t(),!1}),s("#xmas-popup-wrap").on(e,"button#close-xmas-popup",function(t){
//console.log(e);
//xmasBoxClose();
...
*/
s("body").on(e,".faq-nav > button",function(){var t=s(this).data().src;return s(this).siblings().removeClass("btn-active"),s(this).addClass("btn-active"),s(".faq-item.active").animate({top:"100%",opacity:0},300,function(){s(this).removeClass("active")}),s("#"+t).animate({top:"0%",opacity:1},500,function(){s("#"+t).addClass("active")}),!1}),
...
/* VIDEO LINKS BUTTON ACTIONS
	    ...
	    */
s("a.video-link").on(e,function(){var t=s(this).attr("href"),n=s(t).find("video"),e=s(n).attr("id")+"-viewing";return s("body").addClass("video-open"),s("#video-viewer").animate({top:"0px",opacity:1},500,function(){s(this).toggleClass("viewer-closed viewer-open").removeAttr("style"),s(n).clone().attr("id",e).appendTo(".video-viewer-inner"),document.getElementById(e).play()}),!1}),s("button#close-video").on(e,function(){return s(this).parent().animate({top:"100%",opacity:0},500,function(){s(this).toggleClass("viewer-open viewer-closed").removeAttr("style"),s(this).find(".video-viewer-inner").empty(),s("body").removeClass("video-open")}),!1}),
